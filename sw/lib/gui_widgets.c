...
static void label(int x, int y, const char *lbl, const char *val,
                  uint16_t lbl_color, uint16_t val_color) {
    gfx_text(x, y, lbl, lbl_color, 0xFFFF);
    int lx = x + (int)strlen(lbl) * 8;
    gfx_text(lx, y, val, val_color, 0xFFFF);
}

static void draw_bar(int x, int y, int w, int pct, uint16_t fg, uint16_t bg) {
    if (w < 4) return;
...
static void render_shell(tile_t *t, int cx, int cy, int cw, int ch) {
    ...
    for (int i = 0; i < visible; i++) {
        ...
        uint16_t color = FB_LTGRAY;
        if (i == 0) color = FB_GREEN;
        else if (line[0] == ' ') color = FB_LTGRAY;
        gfx_text(cx + 4, cy + 2 + i * 16, line, color, 0xFFFF);
    }

    /* Prompt at bottom */
    if (rows > 0) {
        int py = cy + 2 + max_lines * 16;
        gfx_text(cx + 4, py, "shell > _", FB_GREEN, 0xFFFF);
    }
    ...
    if (cw > 300 && ch > 200) {
        ...
        gfx_text(lx + 8, ly + 4, "NEORV32", FB_CYAN, 0xFFFF);
        gfx_text(lx + 8, ly + 20, "RV32IMC", FB_LTGRAY, 0xFFFF);
        gfx_text(lx + 8, ly + 36, "50 MHz", FB_YELLOW, 0xFFFF);

        /* Freq bar */
        if (ch > 260) {
            gfx_text(lx, ly + 56, "CPU Load:", FB_GRAY, 0xFFFF);
            draw_bar(lx, ly + 72, 112, 23, FB_GREEN, FB_DKGREEN);
            gfx_text(lx, ly + 88, "Memory:", FB_GRAY, 0xFFFF);
            draw_bar(lx, ly + 104, 112, 45, FB_CYAN, FB_DKGREEN);
        }
    }
    (void)cols;
}
...
static void render_monitor(tile_t *t, int cx, int cy, int cw, int ch) {
    ...
    /* Title */
    gfx_text(cx + 4, cy + 4, "System Monitor", FB_CYAN, 0xFFFF);
    ...
    /* Task list header */
    int y = cy + 28;
    gfx_text(cx + 4, y, "TASK", FB_YELLOW, 0xFFFF);
    gfx_text(cx + 120, y, "CPU", FB_YELLOW, 0xFFFF);
    gfx_text(cx + 170, y, "STACK", FB_YELLOW, 0xFFFF);
    gfx_text(cx + 230, y, "HWM", FB_YELLOW, 0xFFFF);
    y += 18;

    /* Task rows */
    for (int i = 0; i < TASK_COUNT && y + 16 < cy + ch; i++) {
        uint16_t name_c = (i < 2) ? FB_LTGRAY : FB_GREEN;
        gfx_text(cx + 4, y, task_names[i], name_c, 0xFFFF);
        ...
        gfx_text(cx + 230, y, "OK", FB_GREEN, 0xFFFF);
        y += 20;
    }
    ...
    if (y + 40 < cy + ch) {
        ...
        y += 8;
        gfx_text(cx + 4, y, "Overall CPU:", FB_LTGRAY, 0xFFFF);
        draw_bar(cx + 4, y + 18, cw - 8, 18, FB_GREEN, FB_DKGREEN);
        y += 38;
        gfx_text(cx + 4, y, "Heap:", FB_LTGRAY, 0xFFFF);
        draw_bar(cx + 4, y + 18, cw - 8, 42, FB_CYAN, FB_DKGREEN);
        y += 38;
        gfx_text(cx + 4, y, "Uptime: 00:42", FB_YELLOW, 0xFFFF);
    }
    ...
    if (ch > 350 && cw > 180) {
        ...
        gfx_text(cx + 4, gy + 4, "CPU History", FB_GRAY, 0xFFFF);
        ...
        int gx = cx + 8;
        int g_y = gy + 22;
        gfx_fill_rect(gx, g_y, gw, gh, FB_GREEN);
        for (int i = 0; i < 20; i++) {
            int bx = gx + i * gw / 20;
            int bw = gw / 20 - 1;
            int bh = spark[i] * gh / 50;
            uint16_t c = spark[i] > 35 ? FB_RED : (spark[i] > 20 ? FB_YELLOW : FB_GREEN);
...
static void render_info(tile_t *t, int cx, int cy, int cw, int ch) {
    (void)t;
    gfx_text(cx + 4, cy + 4, "System Information", FB_YELLOW, 0xFFFF);
    gfx_hline(cx + 4, cy + 22, cw - 8, FB_DKGRAY);
    ...
    /* SoC section */
    gfx_text(cx + 4, y, "[SoC]", FB_CYAN, 0xFFFF);
    y += lh;
    ...
    /* Memory section */
    gfx_text(cx + 4, y, "[Memory]", FB_CYAN, 0xFFFF);
    y += lh;
    ...
    /* Peripherals */
    gfx_text(cx + 4, y, "[Peripherals]", FB_CYAN, 0xFFFF);
    y += lh;
    label(cx + 8, y, "VGA:     ", "640x480 60Hz RGB565", FB_GRAY, FB_LTGRAY);
    y += lh;
    ...
    /* Board */
    if (y + lh < cy + ch) {
        gfx_text(cx + 4, y, "[Board]", FB_CYAN, 0xFFFF);
        y += lh;
    ...
    if (cw > 360 && ch > 200) {
        ...
        gfx_text(rx, ry, "[Palette]", FB_CYAN, 0xFFFF);
        ry += lh;
        uint16_t colors[] = {
        ...
        for (int i = 0; i < 14 && ry + 16 < cy + ch; i++) {
            ...
            gfx_text(rx + 20, ry + 2, cnames[i], FB_LTGRAY, 0xFFFF);
            ry += 18;
        }
        ...
        if (ch > 360) {
            ry += 8;
            gfx_text(rx, ry, "[Box Drawing]", FB_CYAN, 0xFFFF);
            ry += lh;
            /* Draw a box using CP437 chars directly */
            const char *box_top    = "+--+\n";
            const char *box_mid    = "|  |\n";
            const char *box_bottom = "+--+";
            gfx_text(rx + 4, ry, box_top, FB_LTGRAY, 0xFFFF);
            gfx_text(rx + 4, ry + 16, box_mid, FB_LTGRAY, 0xFFFF);
            gfx_text(rx + 4, ry + 32, box_mid, FB_LTGRAY, 0xFFFF);
            gfx_text(rx + 4, ry + 48, box_bottom, FB_LTGRAY, 0xFFFF);
        }
    }
}
