...
static void aes_draw(aes_t *s) {
    ...
    gfx_text(8, 24, buf, CV_DIM, 0xFFFF);
    p = 0;
    p += sprintf(buf+p, "PT:  ");
    for (int i = 0; i < 16; i++) p += sprintf(buf+p, "%02X", s->input[i]);
    gfx_text(8, 40, buf, CV_DIM, 0xFFFF);
    ...
    if (rnd == 0) {
        ...
        gfx_text(30, 58, buf, CV_TXT, 0xFFFF);
        uint16_t col = (s->step == 0) ? CV_WAIT : CV_ACT;
        gfx_rounded_rect(220, 56, 180, 28, 4, col);
        gfx_text(254, 62, "AddRoundKey", CV_TXT, 0xFFFF);
    } else {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            gfx_text(tx, 80, op_name[i], CV_TXT, 0xFFFF);
            if (i < 3)
    ...
    gfx_text(mx, my-14, "State:", CV_TXT, 0xFFFF);
    ...
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            ...
            uint16_t bg = (s->step > 0 && cur[idx] != prv[idx]) ? CV_CHG : CV_WAIT;
            gfx_fill_rect(cx, cy, cw, ch, bg);
            gfx_rect(cx, cy, cw, ch, CV_DIM);
            sprintf(buf, "%02X", cur[idx]);
            gfx_text(cx+7, cy+3, buf, CV_TXT, 0xFFFF);
        }
    }

    int rkx = mx + 4*(cw+2) + 30;
    int show_r = (rnd == 0) ? 0 : rnd;
    gfx_text(rkx, my-14, "Round Key:", CV_TXT, 0xFFFF);
    ...
    if (s->step >= s->max_step) {
        ...
        gfx_text(mx, my + 4*(ch+2)+8, buf, CV_ACT, 0xFFFF);
    }

    gfx_text(8, 340, "Progress:", CV_DIM, 0xFFFF);
...
static void sha_draw(sha_t *s) {
    ...
    gfx_text(8, 28, buf, CV_TXT, 0xFFFF);
    ...
    for (int row = 0; row < 2; row++) {
        for (int col = 0; col < 4; col++) {
            ...
            uint16_t bg = (cv != pv) ? CV_CHG : CV_WAIT;
            gfx_rounded_rect(bx, by, 148, 40, 4, bg);
            sprintf(buf, "%s=%08X", vn[i], cv);
            gfx_text(bx+6, by+12, buf, CV_TXT, 0xFFFF);
        }
    }

    int wy = 170;
    gfx_text(8, wy, "Message Schedule:", CV_TXT, 0xFFFF);
    for (int i = 0; i < 8; i++) {
        int t = s->step - 3 + i;
        int bx = 8 + i*78, by = wy+18;
        if (t < 0 || t >= 64) { gfx_fill_rect(bx,by,74,20,CV_WAIT); continue; }
        uint16_t bg = (i==3) ? CV_ACT : CV_WAIT;
        gfx_fill_rect(bx, by, 74, 20, bg);
        sprintf(buf, "W[%d]", t);
        gfx_text(bx+4, by+2, buf, (i==3)?0:CV_TXT, 0xFFFF);
    }
    if (s->step < 64) {
        sprintf(buf, "W[%d] = 0x%08X", s->step, s->w[s->step]);
        gfx_text(8, wy+44, buf, CV_TXT, 0xFFFF);
    }

    if (s->step >= 64) {
        gfx_text(8, 260, "Digest:", CV_ACT, 0xFFFF);
        int p = 0;
        for (int i = 0; i < 32; i++) p += sprintf(buf+p, "%02x", s->digest[i]);
        gfx_text(8, 278, buf, CV_ACT, 0xFFFF);
    }
