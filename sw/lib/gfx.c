...
#ifndef LOCAL_BUILD
    /* GPU-accelerated path for reasonably sized rects */
    /* Current GPU fill engine writes 8 SDRAM words (=16 RGB565 pixels) per burst
     * and assumes the destination address is 32-bit aligned. Arbitrary x/width
     * rectangles will corrupt adjacent pixels/rows, so only use it for rectangles
     * that satisfy the hardware alignment constraints. */
    if ((w >= 16) && (h >= 1) && ((x & 1) == 0) && ((w & 1) == 0) && ((w % 16) == 0)) {
        uint32_t addr = FRAMEBUFFER_BASE + (y * FB_W + x) * 2;
        gpu_fill_rect(addr, (uint16_t)w, (uint16_t)h,
                      (uint16_t)(FB_W * 2), color);
        gpu_wait();
        return;
    }
#endif
...
void gfx_char(int x, int y, char c, uint16_t fg, uint16_t bg) {
...
void gfx_rounded_rect(int x, int y, int w, int h, int r, uint16_t color) {
...
void gfx_hex_cell(int x, int y, uint8_t value, uint16_t fg, uint16_t bg) {
    gfx_fill_rect(x, y, 16, 16, bg);
    gfx_rect(x, y, 16, 16, fg);
    static const char hx[] = "0123456789ABCDEF";
    gfx_char(x, y, hx[value >> 4], fg, 0xFFFF);
    gfx_char(x + 8, y, hx[value & 0xF], fg, 0xFFFF);
}

void gfx_progress_bar(int x, int y, int w, int h, int cur, int total, uint16_t fg, uint16_t bg) {
...
void gfx_bevel(int x, int y, int w, int h, int raised) {
    uint16_t hi = raised ? FB_WHITE : FB_DKGRAY;
    uint16_t lo = raised ? FB_DKGRAY : FB_WHITE;
...
void gfx_window_frame(int x, int y, int w, int h, const char *title, int active) {
    uint16_t title_bg = active ? FB_BLUE : FB_GRAY;
    int title_h = 18;
    ...
    if (title) {
        ...
        for (int i = 0; i < draw; i++)
            gfx_char(tx + i * FONT_W, y + 4, title[i], FB_WHITE, 0xFFFF);
    }
    ...
    gfx_char(bx + 4, by + 0, 'x', FB_BLACK, 0xFFFF);
