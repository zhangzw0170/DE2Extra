...
**已知 Bug**: NTT TIMEOUT / Snake 边框 (待修)。详见 `doc/phases/de2os-rtos-status.md`。
...
| A6.10 | `gfx_gradient_v()` | 垂直渐变填充 (RGB565 插值)，用于 TWM 背景和窗口内容区 | ✅ LOCAL_BUILD 通过 |
...
| C2.3 | 边界回绕 | 环面拓扑 (硬件取模) | ✅ |
| C2.4 | 迭代推进 | 运行模式下按 speed_ms 间隔推进一代 | ✅ 逐格刷新修复 (2026-06-03) |
| C2.5 | 运行/暂停 | Enter 切换运行/编辑，HUD 右上角显示 RUN/STOP | ✅ |
| C2.6 | 细胞切换 | 编辑态按空格翻转当前光标位置细胞 (硬件 toggle_cell) | ✅ partial refresh 修复 |
| C2.7 | 随机填充 `R` | 按 R 随机初始化网格 | ✅ TRNG 真随机 seed (2026-06-03) |
...
| K. TWM 窗口管理器 | 18 | 13 | 5 | 0 | 0 |
| L. Synth 音频合成器 | 16 | 5 | 11 | 0 | 0 |
| **合计** | **324** | **283** | **29** | **1** | **7** |
...
| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| I.1 | LOCAL_BUILD 编译 | `make local` 零错误零警告通过 | ✅ |
| I.2 | AES NIST 测试向量正确性 | Key=2b7e.../PT=6bc1... → CT=3ad77bb40d7a3660a89ecaf32466ef97 | ✅ |
| I.3 | AES 逐步快照与完整加密一致 | 预计算 41 个快照，逐步累加结果 = aes128_enc_block 输出 | ✅ |
| I.4 | AES pipeline 显示 | 10 轮 × 4 步 (初始轮 2 步, 末轮 3 步)，活跃/已完成/待执行框正确高亮 | ✅ SDL2 |
| I.5 | AES state matrix 变化高亮 | 4×4 字节矩阵，变化字节黄色背景 | ✅ SDL2 |
| I.6 | AES 轮密钥显示 | 右侧同步显示当前轮密钥 | ✅ SDL2 |
| I.7 | AES progress bar | 0/40 → 40/40 递增，显示比例填充 | ✅ SDL2 |
| I.8 | SHA-256 FIPS 测试向量正确性 | "abc" → ba7816bf...f20015ad | ✅ |
| I.9 | SHA-256 逐步快照与完整哈希一致 | 预计算 65 个快照，逐轮结果 = sha256_hash 输出 | ✅ |
| I.10 | SHA-256 工作变量显示 | 2×4 网格 a..h，变化变量黄色高亮 | ✅ SDL2 |

## K. TWM 平铺窗口管理器 (twm.c + gui.c + gfx.c + gui_widgets.c)

> 像素模式桌面环境。Alt 为 Mod 键。VGA 640×480 RGB565。
> 2026-06-03 更新: 移除调试信息 dump，渐变色背景替代纯黑。

### K1. 基本框架

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| K1.1 | `twm` 命令进入 | 输入 `twm` → VGA 切换到像素模式，显示桌面 | ✅ 上板通过 (2026-06-01) |
| K1.2 | VGA 像素模式切回文本 | Esc 退出 TWM → VGA 恢复文本终端 | ✅ 上板通过 |
| K1.3 | 渐变色背景 | 桌面背景为深海军蓝 → 深靛蓝渐变 (非纯黑) | 🟡 代码已完成，待上板验证 |
| K1.4 | 无调试信息输出 | 进入 TWM 时串口不再输出 px regs/dbg/samp 调试信息 | 🟡 代码已完成，待上板验证 |
| K1.5 | 状态栏 | 底部 24px 显示 "DE2Extra" + 当前焦点窗口名 + ZOOM 标记 | ✅ 上板通过 |
| K1.6 | 初始单窗口 | 启动时显示一个 "shell" 叶子窗口，占据全屏 (减去状态栏) | ✅ 上板通过 |
| K1.7 | F1 帮助覆盖 | F1 显示 TWM 快捷键帮助 | 🟡 待上板验证 |
| K1.8 | F10 退出 | F10 退出 TWM 回 shell | 🟡 待上板验证 |

### K2. 平铺操作

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| K2.1 | Alt+H 水平分割 | 将焦点窗口水平分割为两个叶子 | ✅ LOCAL_BUILD 通过 |
| K2.2 | Alt+V 垂直分割 | 将焦点窗口垂直分割为两个叶子 | ✅ LOCAL_BUILD 通过 |
| K2.3 | Alt+Return 自动分割 | 根据窗口宽高比自动选择 H/V 分割方向 | ✅ LOCAL_BUILD 通过 |
| K2.4 | Alt+W 关闭窗口 | 关闭焦点叶子，相邻兄弟接管空间 | ✅ LOCAL_BUILD 通过 |
| K2.5 | 最小窗口数限制 | 仅剩 1 个叶子时 Alt+W 无效 | ✅ LOCAL_BUILD 通过 |
| K2.6 | 最大窗口数限制 | 叶子数达到 TILE_MAX 时不再分割 | ✅ LOCAL_BUILD 通过 |

### K3. 焦点与导航

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| K3.1 | Alt+方向键移动焦点 | Alt+↑↓←→ 将焦点移到相邻叶子 | ✅ LOCAL_BUILD 通过 |
| K3.2 | Tab 循环焦点 | Tab/Shift+Tab 按顺序循环焦点 | ✅ LOCAL_BUILD 通过 |
| K3.3 | 焦点高亮 | 焦点窗口标题栏蓝色，非焦点窗口灰色 | ✅ LOCAL_BUILD 通过 |
| K3.4 | Alt+Shift+方向键调整比例 | 将分割比例 ±5%，范围 10%–90% | ✅ LOCAL_BUILD 通过 |

### K4. 全屏与面板

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| K4.1 | Alt+F 全屏切换 | 焦点窗口全屏/恢复正常 | ✅ LOCAL_BUILD 通过 |
| K4.2 | ZOOM 状态栏标记 | 全屏时状态栏显示 "[ZOOM]" | ✅ LOCAL_BUILD 通过 |
| K4.3 | 面板内容渲染 | 各叶子显示对应 id 的面板内容 (shell/monitor/info) | ✅ LOCAL_BUILD 通过 |
| K4.4 | 分割线 | 相邻叶子间显示 2px 深灰色分割线 | ✅ LOCAL_BUILD 通过 |

## L. Synth 音频合成器 (synth.c + synth_engine.vhd + i2s_tx.vhd + wm8731_ctrl.vhd)

> 双轨合成器: 键盘左侧 → Track 1 (左声道), 数字小键盘 → Track 2 (右声道)。
> 3xOSC (DDS) + DX7 FM 模式。WM8731 codec, I2S 48kHz/16bit。
> 2026-06-03: 有音频输出，软件静音+音量修复已部署，RTL auto-mute 待编译。

### L1. 基本框架

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| L1.1 | `synth` 命令进入 | 输入 `synth` → 进入合成器，VGA 显示键盘布局 | ✅ 串口验证 |
| L1.2 | Codec I2C 初始化 | 串口显示 "Codec I2C ready" | ✅ 串口验证 |
| L1.3 | WB 寄存器写入 | 写 SYNTH_CTRL/T1_NOTE 等寄存器不崩溃 | ✅ 串口验证 |
| L1.4 | F1 帮助覆盖 | F1 显示按键→音阶映射 + 操作说明 | 🟡 待上板验证 |
| L1.5 | F10 退出 | F10 退出 synth 回 shell | 🟡 待上板验证 |
| L1.6 | 退出后无残留音频 | 退出 synth 后耳机完全静音 | 🟡 待上板验证 |

### L2. 音频输出 (需耳机/音箱)

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| L2.1 | 按键有音调输出 | 按住 A 键 → 耳机听到 C4 (262Hz) 音调 | ✅ 物理验证通过 (2026-06-03) |
| L2.2 | 无键按下时完全静音 | 不按键 → 耳机无任何声音 (无高频背景音) | 🟡 SW mute 已部署，待物理验证 |
| L2.3 | 松键立即停止 | 松开按键 → 音符立即停止，不持续 | 🟡 SW mute 已部署，待物理验证 |
| L2.4 | 多键和弦 | 同时按多个键 → 听到和弦 (双轨) | 🟡 待上板验证 |
| L2.5 | Track 1 键盘映射 | A=C4, W=C#4, S=D4, ... 对应两个八度 | 🟡 待上板验证 |
| L2.6 | Track 2 小键盘映射 | 小键盘数字键对应另一组音阶 | 🟡 待上板验证 |

### L3. 音量控制

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| L3.1 | ↑ 键增大音量 | 按 ↑ 音量增大 (master_vol 减小) | 🟡 SW 已部署，待物理验证 |
| L3.2 | ↓ 键减小音量 | 按 ↓ 音量减小 (master_vol 增大) | 🟡 SW 已部署，待物理验证 |
| L3.3 | 音量条显示 | 右上角显示 Vol: + 竖线音量条 (4 级) | 🟡 待上板验证 |

### L4. RTL 硬件验收 (synth_engine.vhd + i2s_tx.vhd)

| # | 验收项 | 预期行为 | 状态 |
|---|---|---|---|
| L4.1 | I2S BCLK/LRCK 时序 | BCLK=4MHz (18MHz/4-1 div), LRCK=48kHz | ✅ 逻辑分析仪/示波器验证 |
| L4.2 | WM8731 寄存器配置 | R4=0x08F8, R7=0x0E01, R8=0x1002 对齐 Terasic 参考 | ✅ I2C 写入验证 |
| L4.3 | 寄存器读回 | CPU 可读回所有 14 个 WB 寄存器 | 🟡 RTL 已改，待编译验证 |
| L4.4 | Auto-mute (RTL) | any_note_active=0 时 DDS 输出强制归零 | 🟡 RTL 已改，待编译验证 |
| L4.5 | altpll_audio 18MHz | PLL 50MHz→18MHz 稳定输出 (VCO=900MHz) | ✅ Quartus 编译通过 |

*最后更新: 2026-06-03 — 总计 ~324 项 (283✅ / 29🟡 / 1❌ / 7N/A)。已知 Bug 详见 `doc/phases/de2os-rtos-status.md`。*
