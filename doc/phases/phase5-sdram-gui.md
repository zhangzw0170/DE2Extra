...
☑ VHDL 已完成 (`vga_pixel_ctrl.vhd`)。⚠️ de2os_top 中已声明但**信号全部禁用** (tied to zero)——需要接线。🔌 待上板。
...
1. 在 de2os_top 中将 vga_pixel_ctrl 的寄存器信号接到 wb_intercon（当前全禁用）
2. VGA 输出 MUX：vga_pixel_mode 信号选择 text/pixel 输出到 VGA 引脚
...
- ☑ Wishbone 集成: wb_intercon s4, 基地址 0xF000C000
...
+----------+                     +-----------------+
| de2os    |-- XBUS --> WM8731   | WM8731 CODEC    |
...
| **V3 合计 (不含双核)** | **~10,005** | **~8** | **~39** | — |
| 板载资源 | 114,480 | 532 | 432 | — |
| 占用率 | **8.7%** | **1.5%** | **9.0%** | 极度充裕 |
...
|--------|--------|----------|----------|------|
| **P0** | 1. SDRAM 执行 + bootloader | ~~4h~~ → 0.5h | ☑ de2os.sof 已编译 (8,125 LEs, 无时序违例) | 🔌 上板 |
...
| **P1** | 5. FreeRTOS 移植 | ~~4h~~ → 0.5h | ☑ 4 任务已注册 (uart_input/shell/active/status) | 🔌 上板 |
| **P1** | 2+4. VGA 像素模式 + 图元库 | ~~5h~~ → 1.5h | ☑ vga_pixel_ctrl.vhd 已编译，⚠️ de2os_top 信号全禁用需接线 | 接线 + 🔌 上板 |
...
| **P3** | 9. NTT 加速器 | ~~8h~~ → 1h | ☑ ntt_sdf.vhd 编译通过 (在 de2os 中) + C 驱动 + Python 验证 | 🔌 上板 |
...
| **总计 (P0-P3)** | | **~27h** (原 47h) | 已完成 ~20h，剩余 ~27h | |

### 13. FreeRTOS+CLI 集成 (🆕)

**前置**: 更新 FreeRTOS submodule 从 V11.1.0+ (开发分支) 到 **V11.3.0** (最新 release，2026-03-30)。不追 main 分支。
