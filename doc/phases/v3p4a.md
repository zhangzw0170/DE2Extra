...
| 编号 | 验收项 | 状态 |
|------|--------|------|
| V3P4A.S1.1 | QSF 添加 conway_engine.vhd 和 pong_engine.vhd，Quartus 编译零错误零警告（除 NEORV32 内部警告外） | ⬜ |
| V3P4A.S1.2 | de2os_top.vhd 删除 stub 赋值，替换为 pong_engine + conway_engine 真实实例化，端口映射正确 | ⬜ |
...
| V3P4A.S4.2 | C 驱动 make local 编译通过（de2shell_rtos 交叉编译通过） | ⬜ |
