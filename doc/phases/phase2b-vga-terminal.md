...
>
> **阻塞项 (2026-05-23 更新)**:
> - ~~VGA/PS/2 外设已接入 `de2_115_top.vhd` 和 `wb_intercon`~~ ✅
> - ~~QSF 中 VGA/PS/2 引脚分配已完成~~ ✅
> - ⏳ **物理连线**: 现有的是 HDMI→VGA 线 (单向, 方向反), 需 VGA→HDMI 有源转换器。预计 5/26 (周一) 到货后验证 640×480 兼容性
>
> **显示方案**: DE2-115 输出标准 640×480@60Hz VGA → VGA→HDMI 有源转换器 → HDMI 显示器。FPGA 端无需修改分辨率。
