...
| 范围            | 用途         | 说明                    |
|-----------------|--------------|-------------------------|
| 0x00000000      | IMEM         | 32KB 指令存储            |
| 0x80000000      | DMEM         | 16KB 数据存储            |
| 0x01000000      | SDRAM        | 128MB (XBUS → wb_intercon) |
| 0xF0000000      | VGA 终端     | 8KB，Phase 2b            |
| 0xF0002000      | PS/2 键盘    | 4KB，Phase 2b            |
| 0xF0004000      | 系统定时器   | 4KB，预留                |
| 0xF0006000      | 中断控制器   | 4KB，预留                |
| 0xF0008000      | LCD 控制器   | 4KB，已实现              |
| 0xF0009000      | IR 接收器    | 4KB，Phase 3             |
| 0xF000A000      | DDS 音频     | 4KB，Phase 3             |
...
```
src/rtl/de2_115_top.vhd      — 板级顶层，引脚映射
src/rtl/neorv32_wrapper.vhd  — CPU 封装，55 个 generic 配置
src/rtl/glue/clk_rst_gen.vhd — PLL + 复位生成
src/rtl/bus/wb_intercon.vhd  — Wishbone 总线互连，地址解码
src/rtl/periph/sdram_ctrl.vhd— SDRAM 控制器 (100MHz 状态机)
src/rtl/periph/seg7_mapper.vhd— 七段管译码
src/rtl/periph/lcd_status.vhd — LCD HD44780 驱动
...
- **IMEM 大小**: 32KB，固件不能超过这个限制（`neorv32_image_gen` 会报错）
