...
- `par/de2os/de2os.qsf` — 已添加 conway_engine.vhd 和 pong_engine.vhd (2026-05-29)
- `src/rtl/de2os_top.vhd` — **stub 已替换为真实实例化** (2026-05-29):
  - u_pong: entity work.pong_engine (完整 Wishbone + VGA 端口)
  - u_conway: entity work.conway_engine (完整 Wishbone 端口)
  - VGA MUX 三级优先级已确认 (PONG > pixel > text)
- `sw/app/de2shell_rtos/main.c` — conwayhw/ponghw CLI 命令已注册 (2026-05-29)

### 需要完成

1. ~~`par/de2os/de2os.qsf` — 添加 `conway_engine.vhd` 和 `pong_engine.vhd`~~ ☑
2. ~~`de2os_top.vhd` — 删除 stub 赋值，替换为真实组件实例化~~ ☑
3. ~~`wb_intercon.vhd` — 已完成，无需修改~~ ☑
4. 仿真验证 — 需编写或确认 Conway/PONG 仿真测试
5. ~~C 驱动编译 — 已在 makefile 中，交叉编译通过~~ ☑
6. ~~CLI 命令注册 — conwayhw/ponghw 已注册~~ ☑
7. 上板验证 — 待板子到手

## 3. 实现步骤

### 步骤 1: QSF 添加 VHDL 文件 — ☑ 已完成 (2026-05-29)

已在 `par/de2os/de2os.qsf` 中添加：
```qsf
...
### 步骤 2: de2os_top.vhd 替换 stub — ☑ 已完成 (2026-05-29)

删除第 743-757 行的全部 stub 赋值代码，替换为两个组件实例化。实际代码与下方设计一致。
...
| 编号 | 验收项 | 状态 |
|------|--------|------|
| V3P4A.S1.1 | QSF 添加 conway_engine.vhd 和 pong_engine.vhd，Quartus 编译通过 | ☑ (0 errors, 262 warnings) |
| V3P4A.S1.2 | de2os_top.vhd stub 替换为 pong_engine + conway_engine 真实实例化 | ☑ |
| V3P4A.S2.1 | Conway 仿真：复位后 status=0，randomize 后 grid_row 非零，step 后 generation=1 | ⬜ |
| V3P4A.S2.2 | PONG 仿真：VGA 时序参数正确，发球后 ball 位置变化，scores 寄存器可读 | ⬜ |
| V3P4A.S3.1 | wb_intercon s9/s10 片选和地址映射已确认正确 | ☑ (无需修改) |
| V3P4A.S3.2 | VGA MUX 三级优先级：PONG > pixel > text | ☑ |
| V3P4A.S4.1 | pong_hw.c PS2_MMIO_BASE 与 de2extra_pkg.vhd 一致 | ☑ |
| V3P4A.S4.2 | C 驱动 de2shell_rtos 交叉编译通过 | ☑ |
| V3P4A.S6.1 | conwayhw CLI 命令注册成功 | ☑ |
| V3P4A.S6.2 | ponghw CLI 命令注册成功 | ☑ |
| V3P4A.S5.1 | Quartus 编译报告：LE/M9K 增量在预期范围内 | ⬜ (需查 fit report) |
| V3P4A.S5.2 | 时序分析：50MHz 和 25MHz 路径无负 slack | ☑ (worst setup +2.394ns) |
