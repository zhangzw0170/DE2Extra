...
| V3P3A.S3.1 | de2shell_rtos 构建包含 GUI 源文件, Docker 交叉编译零错误 | ✅ 2026-05-31 编译 149KB 固件成功 |
| V3P3A.S3.2 | 固件大小合理 (<100KB @ SDRAM) | ✅ 149KB, SDRAM 128MB 空间充足 |
...
| V3P3A.S5.1 | FPGA 像素模式: VGA 显示 RGB 渐变填充 | 🟡 TWM 已显示内容但花屏，需 pxtest 进一步诊断 |
| V3P3A.S5.2 | FPGA 像素模式: Win 3.0 桌面完整渲染 (9 图标 + 2 窗口 + 任务栏) | 🟡 TWM 进入后物理显示器有画面但刷新慢/花屏 |
| V3P3A.S5.3 | FPGA 像素模式: PS/2 键盘输入驱动桌面交互 | ⬜ 无 PS/2 转接器 |
| V3P3A.S6.1 | 文本模式 -> 像素模式切换正常 (win30 启动) | ✅ twm 进入后物理显示器切换到像素模式画面 |
| V3P3A.S6.2 | 像素模式 -> 文本模式切换正常 (ESC 退出后 shell 恢复) | ⬜ twm ESC 后板子卡住，需重置 |
| V3P3A.S6.3 | 文本/像素模式可反复切换 >=3 次, 无状态异常 | ⬜ |
| V3P3A.S7.1 | Quartus 编译通过, LE 占用 <11,000 (含 vga_pixel_ctrl) | ✅ 71,299 LEs total (62%) |
| V3P3A.S7.2 | 单帧渲染 (clear + gui_render_all) < 50ms | ⬜ |
| V3P3A.S7.3 | SDRAM 帧缓冲不与代码段/堆重叠 (链接脚本确认) | ✅ 代码@0x01000000, FB@0x01800000, heap@0x01900000 |
...
| V3P3A.S10.2 | `twm` CLI 命令已注册 (替换 `win30`/`startui`/`gui` 别名) | ✅ 2026-05-31 help 列表可见 twm |
