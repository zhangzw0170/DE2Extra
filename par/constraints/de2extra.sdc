...
# VGA 25MHz pixel clocks (derived from 50MHz toggle in each VGA module)
# Each module generates clk_25m by toggling on rising_edge(clk_50m_i).
create_generated_clock -name clk_25m_txt -divide_by 2 \
    -source [get_pins {u_vga|clk_25m|clk}] \
    [get_pins {u_vga|clk_25m|q}]
create_generated_clock -name clk_25m_px -divide_by 2 \
    -source [get_pins {u_vga_px|clk_25m|clk}] \
    [get_pins {u_vga_px|clk_25m|q}]
create_generated_clock -name clk_25m_pong -divide_by 2 \
    -source [get_pins {u_pong|clk_25m|clk}] \
    [get_pins {u_pong|clk_25m|q}]
